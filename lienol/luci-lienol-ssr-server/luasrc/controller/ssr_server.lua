...
entry({"admin","vpn","ssr_server"},cbi("ssr_server/users"),_("SSR Server"),46).dependent=true
entry({"admin","vpn","ssr_server","status"},call("act_status")).leaf=true
end

function act_status()
  local e={}
  e.ssr_server_status=luci.sys.call("ps | grep server.py |grep -v grep >/dev/null")==0
